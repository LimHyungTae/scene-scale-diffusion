...
class CarlaDataset(Dataset):
    ...
    def __init__(
        ...
        param_file = os.path.join(
            self._directory, self._scenes[0], "evaluation", "params.json"
        )
        ...
        for scene in self._scenes:
            ...
            frames_list = [
                ...
                for filename in sorted(os.listdir(velodyne_dir))
            ]
            self._frames_list.extend(frames_list)
            self._velodyne_list.extend(
                [
                    os.path.join(velodyne_dir, str(frame).zfill(6) + ".bin")
                    for frame in frames_list
                ]
            )
            self._label_list.extend(
                [
                    os.path.join(label_dir, str(frame).zfill(6) + ".label")
                    for frame in frames_list
                ]
            )
            self._pred_list.extend(
                [
                    os.path.join(pred_dir, str(frame).zfill(6) + ".bin")
                    for frame in frames_list
                ]
            )
            self._eval_labels.extend(
                [
                    os.path.join(eval_dir, str(frame).zfill(6) + ".label")
                    for frame in frames_list
                ]
            )
            self._eval_counts.extend(
                [
                    os.path.join(eval_dir, str(frame).zfill(6) + ".bin")
                    for frame in frames_list
                ]
            )
            self._timestamps.append(
                np.loadtxt(os.path.join(self._directory, scene, "times.txt"))
            )
            self._poses.append(
                np.loadtxt(os.path.join(self._directory, scene, "poses.txt"))
            )
            # for poses and timestamps
        self._timestamps = np.array(self._timestamps).reshape(
            sum(self._num_frames_scene)
        )
        self._poses = np.array(self._poses).reshape(sum(self._num_frames_scene), 12)

        self._cum_num_frames = np.cumsum(
            np.array(self._num_frames_scene) - self._num_frames + 1
        )
    ...
    def points_to_voxels(self, voxel_grid, points, t_i):
        ...
        if self.binary_counts:
            ...
            voxel_grid[
                t_i, unique_voxels[:, 0], unique_voxels[:, 1], unique_voxels[:, 2]
            ] += counts
        return voxel_grid
    ...
    def __getitem__(self, idx):
        ...
        if self.voxelize_input:
            voxel_input = np.zeros(
                (
                    idx_range.shape[0],
                    int(self.grid_dims[0]),
                    int(self.grid_dims[1]),
                    int(self.grid_dims[2]),
                ),
                dtype=np.float32,
            )
        t_i = 0
        ...
        for i in idx_range:
            if i == -1:  # Zero pad
                ...
                points = np.fromfile(self._velodyne_list[i], dtype=np.float32).reshape(
                    -1, 4
                )[:, :3]
                ...
                if self.transform_pose:
                    ...
                    points = (
                        np.dot(relative_pose[:3, :3], points.T).T + relative_pose[:3, 3]
                    )
        ...
        if self.get_gt:
            output = (
                np.fromfile(self._eval_labels[idx_range[-1]], dtype=np.uint32)
                .reshape(self._eval_size)
                .astype(np.uint8)
            )
            counts = np.fromfile(
                self._eval_counts[idx_range[-1]], dtype=np.float32
            ).reshape(self._eval_size)
        else:
            output = None
            counts = None
        ...
        if self.voxelize_input and self.random_flips:
            ...
            if np.random.randint(2):
                ...
                voxel_input = np.flip(
                    voxel_input, axis=1
                )  # Because there is a time dimension
            # Y Flip
            if np.random.randint(2):
