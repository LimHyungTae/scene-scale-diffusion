...
class Con_Diffusion(torch.nn.Module):
    def __init__(
        ...
        if args.dataset == "carla":
            ...
            self._denoise_fn = Denoise(
                args=self.args, num_class=self.num_classes, init_size=16
            )
    ...
    def sample(self, voxel_input, intermediate=False):
        ...
        uniform_logits = torch.zeros(
            (self.args.batch_size, self.num_classes) + self.shape, device=device
        )
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            log_model_prob, log_x0_recon = self.p_pred(
                log_x=log_z, t=t, cond=voxel_input
            )
