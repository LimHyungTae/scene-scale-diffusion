...
class Asymmetric_Residual_Block(nn.Module):
    def __init__(self, in_filters, out_filters, time_filters=32 * 4):
        ...
        self.time_layers = nn.Sequential(
            nn.SiLU(), nn.Linear(time_filters, in_filters * 2)
        )
...
class DownBlock(nn.Module):
    def __init__(
        ...
        self.residual_block = Asymmetric_Residual_Block(
            in_filters, out_filters, time_filters=time_filters
        )
...
class Denoise(nn.Module):
    def __init__(self, args, num_class=11, init_size=32, discrete=True):
        ...
        self.A = Asymmetric_Residual_Block(
            init_size, init_size, time_filters=init_size * 4
        )

        self.downBlock1 = DownBlock(
            init_size, 2 * init_size, height_pooling=True, time_filters=init_size * 4
        )
        self.downBlock2 = DownBlock(
            2 * init_size,
            4 * init_size,
            height_pooling=True,
            time_filters=init_size * 4,
        )
        self.downBlock3 = DownBlock(
            4 * init_size,
            8 * init_size,
            height_pooling=False,
            time_filters=init_size * 4,
        )
        self.downBlock4 = DownBlock(
            8 * init_size,
            16 * init_size,
            height_pooling=False,
            time_filters=init_size * 4,
        )
        self.midBlock1 = Asymmetric_Residual_Block(
            16 * init_size, 16 * init_size, time_filters=init_size * 4
        )
        self.attention = Attention(16 * init_size, 32)
        self.midBlock2 = Asymmetric_Residual_Block(
            16 * init_size, 16 * init_size, time_filters=init_size * 4
        )
    ...
    def forward(self, x, x_cond, t):
        ...
        try:
            ...
            print("\033[1;31mError during torch.cat:")
            print("x shape:", x.shape)
            print("x_cond shape:", x_cond.shape, "\033[0m")
            raise e  # Re-raise to keep original traceback
