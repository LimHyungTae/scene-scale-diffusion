...
class DownBlock(nn.Module):
    def __init__(
        self, in_filters, out_filters, pooling=True, drop_out=True, height_pooling=False
    ):
...
class UpBlock(nn.Module):
    def __init__(self, in_filters, out_filters, height_pooling):
        ...
        if height_pooling:
            self.up_subm = nn.ConvTranspose3d(
                out_filters,
                out_filters,
                kernel_size=3,
                bias=False,
...
class C_Encoder(nn.Module):
    def __init__(self, args, nclasses=20, init_size=16, l_size="882", attention=True):
        ...
        if self.l_size == "32322":
            ...
            self.out = nn.Conv3d(
                4 * init_size, nclasses, kernel_size=3, stride=1, padding=1, bias=True
            )
            ...
            self.out = nn.Conv3d(
                8 * init_size, nclasses, kernel_size=3, stride=1, padding=1, bias=True
            )
            ...
            self.out = nn.Conv3d(
                16 * init_size, nclasses, kernel_size=3, stride=1, padding=1, bias=True
            )
        else:
    ...
    def __init__(
        ...
        if l_size == "882":
            self.conv_in = nn.Conv3d(
                nclasses, 16 * init_size, kernel_size=3, stride=1, padding=1, bias=True
            )
            self.midBlock1 = Asymmetric_Residual_Block(16 * init_size, 16 * init_size)
            self.attention = Attention(16 * init_size, 32)
            self.midBlock2 = Asymmetric_Residual_Block(16 * init_size, 16 * init_size)
            ...
            self.conv_in = nn.Conv3d(
                nclasses, 8 * init_size, kernel_size=3, stride=1, padding=1, bias=True
            )
            self.midBlock1 = Asymmetric_Residual_Block(8 * init_size, 8 * init_size)
            self.attention = Attention(8 * init_size, 32)
            self.midBlock2 = Asymmetric_Residual_Block(8 * init_size, 8 * init_size)
            ...
            self.conv_in = nn.Conv3d(
                nclasses, 4 * init_size, kernel_size=3, stride=1, padding=1, bias=True
            )
            self.midBlock1 = Asymmetric_Residual_Block(4 * init_size, 4 * init_size)
            self.attention = Attention(4 * init_size, 32)
            self.midBlock2 = Asymmetric_Residual_Block(4 * init_size, 4 * init_size)
        ...
        # Original Carla dataset needs it,
        # but for 3DFront, whose last channel is 1, the upsampling is not needed
        if use_height_pooling:
            ...
            self.upBlock2 = UpBlock(4 * init_size, 2 * init_size, height_pooling=False)
            self.upBlock1 = UpBlock(2 * init_size, 2 * init_size, height_pooling=False)

        self.DDCM = DDCM(2 * init_size, 2 * init_size)
        self.logits = nn.Conv3d(
            4 * init_size, self.nclasses, kernel_size=3, stride=1, padding=1, bias=True
        )
...
class Completion(nn.Module):
    def __init__(self, args, num_class=11, init_size=32):
        ...
        self.DDCM = DDCM(2 * init_size, 2 * init_size)
        self.logits = nn.Conv3d(
            4 * init_size, self.num_class, kernel_size=3, stride=1, padding=1, bias=True
        )
