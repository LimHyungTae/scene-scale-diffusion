...
def inter_vis(args, recons):
    for r in range(len(recons)):
        for batch, samples_i in enumerate(recons[r]):
            ...
            np.savetxt(
                "/home/jumin/multinomial_diffusion/Result/Condition/Completion/iteration/batch{}_{}.txt".format(
                    batch, r
                ),
                colors_indexs,
            )
...
def visualization(args, recons, input_data, output, invalid, iteration):
    for batch, (samples_i, input_i, output_i, invalid_i) in enumerate(
        zip(recons, input_data, output, invalid)
    ):
        color_index = []
        output_index = []
        ...
        np.savetxt(
            args.log_path
            + "/Completion/result_{}.txt".format((iteration * args.batch_size) + batch),
            colors_indexs,
        )
...
def completion_vis(args, input_p, recons):
    for batch, (recon_i, input_i) in enumerate(zip(recons, input_p)):
        ...
        input_points = torch.nonzero(input_i == 1, as_tuple=False).cpu().numpy()
        np.savetxt(
            args.log_path + "/Completion/completion_{}.txt".format(batch), recon_points
        )
...
def get_result(args, for_mask, output, preds, SSC=True):
    ...
    if SSC:
        if args.dataset == "kitti":
            mask = for_mask == 0
        elif args.dataset == "carla" or args.dataset == "3dfront":
            mask = for_mask > 0
    else:
        mask = for_mask == 1
    ...
    intersection, union = iou_one_frame(
        preds_masked, output_masked, n_classes=args.num_classes
    )
