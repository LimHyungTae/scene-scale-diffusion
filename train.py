...
class Experiment(object):
    ...
    def __init__(
        ...
        self.optimizer, self.scheduler_iter, self.scheduler_epoch = (
            optimizer,
            scheduler_iter,
            scheduler_epoch,
        )
        ...
        config_file = os.path.join(f"./datasets/{args.dataset}.yaml")
        if os.path.exists(config_file):
            with open(config_file, "r") as f:
                configs = yaml.safe_load(f)
            self.color_map = configs["remap_color_map"]
            self.remap = None
            LABEL_TO_NAMES = configs["label_to_names"]
        ...
        # Store data loaders
        self.train_loader, self.eval_loader, self.test_loader, self.train_sampler = (
            train_loader,
            eval_loader,
            test_loader,
            train_sampler,
            ...
            if self.args.log_tb:
                ...
                if ssc_dict:
                    ...
                    self.writer.add_text(
                        "SSC_mIoU",
                        get_miou_table(
                            self.args, self.label_to_names, miou
                        ).get_html_string(),
                        global_step=epoch + 1,
                    )
                    for metric_name, metric_value in seg_dict.items():
                        self.writer.add_scalar(
                            "Seg/{}".format(metric_name),
                            metric_value,
                            global_step=epoch + 1,
                        )
                    self.writer.add_text(
                        "Seg_mIoU",
                        get_miou_table(
                            self.args, self.label_to_names, seg_miou
                        ).get_html_string(),
                        global_step=epoch + 1,
                    )
    ...
    def sample(self):
        ...
        with torch.no_grad():
            ...
            s_TP, s_FP, s_TN, s_FN, s_num_correct, s_num_total = (
                0.0,
                0.0,
                0.0,
                0.0,
            ...
            all_intersections, all_unions = (
                np.zeros(self.args.num_classes),
                np.zeros(self.args.num_classes) + 1e-6,
            )
            s_all_intersections, s_all_unions = (
                np.zeros(self.args.num_classes),
                np.zeros(self.args.num_classes) + 1e-6,
            )
            if self.args.dataset == "carla":
                dataloader = self.test_loader
            # TODO(hlim): Has to check whether using `self.test_loader` is correct or not
            elif self.args.dataset == "3dfront":
                dataloader = self.test_loader
            else:
            ...
            for iterate, (voxel_input, output, counts) in enumerate(dataloader):
                if len(voxel_input) == self.args.batch_size:
                    ...
                    (
                        ...
                        intersection,
                        union,
                    ) = get_result(self.args, invalid, output, recons)
                    ...
                    (
                        ...
                        s_intersection,
                        s_union,
                    ) = get_result(self.args, voxel_input, output, recons, SSC=False)
            ...
            return (
                {"IoU": iou, "mIoU": np.mean(miou) * 100},
                miou,
                {"IoU": s_iou, "mIoU": np.mean(seg_miou) * 100},
                seg_miou,
            )
    ...
    def resume(self):
        ...
        for epoch in range(self.current_epoch):
            ...
            if epoch in self.completion_epochs:
                sample_dict = {}
                for metric_name, metric_values in self.eval_metrics.items():
                    sample_dict[metric_name] = metric_values[
                        self.eval_epochs.index(epoch)
                    ]
            else:
                sample_dict = None
            ...
            for metric_name, metric_value in train_dict.items():
                self.writer.add_scalar(
                    "base/{}".format(metric_name), metric_value, global_step=epoch + 1
                )
            if eval_dict:
                for metric_name, metric_value in eval_dict.items():
                    self.writer.add_scalar(
    ...
    def checkpoint_save(self, epoch):
        if self.args.distribution:
            checkpoint = {
                ...
                "scheduler_iter": self.scheduler_iter.state_dict()
                if self.scheduler_iter
                else None,
                "scheduler_epoch": self.scheduler_epoch.state_dict()
                if self.scheduler_epoch
                else None,
            }
        else:
