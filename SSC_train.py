...
NODE_RANK = (
    os.environ["AZ_BATCHAI_TASK_INDEX"] if "AZ_BATCHAI_TASK_INDEX" in os.environ else 0
)
...
def get_args():
    ...
    parser.add_argument(
        "--node_rank", type=int, default=0, help="node rank for distributed training"
    )
    parser.add_argument(
        "--dist_url",
        type=str,
    ...
    # Data params
    parser.add_argument(
        "--dataset", type=str, default="carla", choices=["carla", "3dfront"]
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mode", default="wo_diff", choices="gen, con, vis, l_vae l_gen, wo_diff"
    )
...
def start(local_rank, args):
    ...
    ########################
        ...
        model = Diffusion(
            args, completion_criterion, auxiliary_loss_weight=args.auxiliary_loss_weight
        ).cuda()
        if args.distribution:
            model = torch.nn.parallel.DistributedDataParallel(
                model, device_ids=[args.gpu], find_unused_parameters=False
    ...
    ######################
        ...
        if args.distribution:
            Dense = torch.nn.parallel.DistributedDataParallel(
                Dense, device_ids=[args.gpu], find_unused_parameters=False
            )
